...
class Services extends Ana_Controller
{
    ...
    function index()
    {
            ...
            if ($_POST):
                ...
                elseif (!is_numeric($price)):
                    ...
                    if ($update):
                        ...
                        if ($max != $serviceInfo["service_min"]) {
                            if ($serviceInfo["service_type"] == 2) {
                                $servicereport->save(array(
                                    'service_id' => route(4),
                                    'alert' => $serviceInfo["service_name"],
                                    'extra' => json_encode([
                                        'action' => $max > $serviceInfo["service_min"] ? 'min_up' : 'min_down',
                                        'old' => $serviceInfo["service_min"],
                                        'new' => $max
                                    ])
                                ));
                        ...
                        $errorText = "İşlem başarılı";
                        $icon = "success";
                        $referrer = $this->request->getGet('modalsave') ? -1 : base_url("admin/services");
                    else:
                        $conn->rollBack();
                        $error = 1;
                        $errorText = "İşlem başarısız";
        ...
        elseif ($action == "edit-service-aciklama" && $action):
            $category_id = route(4);
            if (!countRow(["table" => "services", "where" => ["service_id" => $category_id]])): header("Location:" . base_url("admin/services"));
                exit(); endif;
