...
if( !route(2) ){
  ...
  if( $_POST ){
    ...
    $message  = htmlentities($_POST["message"]);
      if( empty($subject) ){
        ...
        $insert2= $conn->prepare("INSERT INTO ticket_reply SET client_id=:c_id,ticket_id=:t_id, message=:message, time=:time ");
        $insert2= $insert2->execute(array("c_id"=>$user["client_id"],"t_id"=>$ticket_id,"message"=>$message,"time"=>date("Y.m.d H:i:s")));
      ...
      if($post){
        ...
        if( $insert && $insert2 && $insert3 ):
          unset($_SESSION["data"]);
          header('Location:'.base_url('tickets/')."/".$ticket_id);
          $conn->commit();
...
}elseif( route(2) && preg_replace('/[^0-9]/', '', route(2)) && !preg_replace('/[^a-zA-Z]/', '', route(2))  ){
  ...
  if(new_ticket($user['client_id'])){
    ...
    $ticketUpdate-> execute(array("c_id"=>$user["client_id"], "new"=>1, "t_id"=>route(2) ));
  }

  $messageList  = $conn->prepare("SELECT * FROM ticket_reply WHERE ticket_id=:t_id ");
  $messageList  -> execute(array("t_id"=>route(2)));
  ...
  $ticketList-> execute(array("c_id"=>$user["client_id"], "t_id"=>route(2) ));
  ...
  if( $_POST ){
   ...
   $message  = htmlspecialchars($_POST["message"]);
      if( strlen(str_replace(' ','',$message)) < 5 ){
        ...
        $update = $update->execute(array("last_time"=>date("Y.m.d H:i:s"),"t_id"=>route(2),"new"=>2,"status"=>"pending" ));
        $insert = $conn->prepare("INSERT INTO ticket_reply SET client_id=:c_id,ticket_id=:t_id, message=:message, time=:time ");
        $insert = $insert->execute(array("c_id"=>$user["client_id"],"t_id"=>route(2),"message"=>$message,"time"=>date("Y.m.d H:i:s")));
        $insert3= $conn->prepare("INSERT INTO client_report SET client_id=:c_id, action=:action, report_ip=:ip, report_date=:date ");
        $insert3= $insert3->execute(array("c_id"=>$user["client_id"],"action"=>"Support request responded #".route(2),"ip"=>GetIP(),"date"=>date("Y-m-d H:i:s") ));
        if( $update && $insert && $insert3 ):
          unset($_SESSION["data"]);
          $conn->commit();
          header("Location:".base_url('tickets/')."/".route(2));
        else:
          $error    = 1;
...
}elseif( route(2) && preg_replace('/[^a-zA-Z]/', '', route(2))  ){
