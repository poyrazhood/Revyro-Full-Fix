...
if ($_POST):
    ...
    elseif (($user["u_balance"] - $price < "-" . $user["debit_limit"]) && $user["balance_type"] == 1):
        ...
        if ($service_detail['rep_link'] == 2 && $order_ss->where(["client_id" => $user["client_id"], 'order_url' => $link, 'order_status!=' => 'completed', 'service_id' => $service_detail['service_id']])->where([ 'order_status!=' => 'canceled'])->where(['order_status!=' => 'partial'])->countAllResults()) {
