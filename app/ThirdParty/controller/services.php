...
$categories = [];
  ...
  foreach ( $categoriesRows as $categoryRow ) {
    ...
    $search->execute(array("category"=>$categoryRow["category_id"],"c_id"=>$login?$user["client_id"]:0));
       ...
       foreach ( $rows as $row ) {
        ...
        if($settings["avarage"] == 2):      
           ...
           $multiDesc   =  json_decode($row["description_lang"],true);
            ...
            if( $login && isset($multiDesc[$user["lang"]]) ):
          ...
          $search->execute(array("service"=>$row["service_id"],"c_id"=>$login?$user["client_id"]:0));
